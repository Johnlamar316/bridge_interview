...
interface CheckboxSelectProps
  extends Omit<HTMLProps<HTMLLabelElement>, 'onChange' | 'value'> {
  className?: string;
  onChange?: (
...
export const CheckboxSelect = (props: CheckboxSelectProps) => {
  ...
  const onChecked = (e: ChangeEvent<HTMLInputElement>) => {
    if (onChange) {
      onChange({ checked: e.target.checked, name: e.target.name }, e);
      e.stopPropagation()
    }
  };
  return (
    <label
      ...
      className={`${className ?? ''} -checkbox-select ${
        $checked ? '-checked' : ''
      }`}
      {...rest}
    >
      <input
        ...
        onChange={onChecked}
      />
      <span className="-checkbox-select-inner -flex -flex-between -flex-a-center">
        <svg
        ...
        <span className="-checkbox-select-label">{label || children}</span>
        <span className="-checkbox-select-estimate -flex -flex-center-col">
          <span>Estimate</span>
          <span>{estimate}</span>
        </span>
      </span>
